...
<body>
    ...
    <script>
        ...
        pdf.addEventListener('click', async () => {
            <?php if ($_ENV['MIRUN_PLATFORM'] == 'linux') { ?>
                await mirun.exportPDF('<?php echo dirname(__FILE__) . '/pdf/example.pdf'; ?>');
            <?php } else { ?>
                await mirun.exportPDF('<?php echo str_replace('\\','\\\\', dirname(__FILE__)) . '\\\\pdf\\\\example.pdf'; ?>');
            <?php } ?>
            mirun.newWindow('/pdf/example.pdf');
        });
    </script>
</body>
