...
<body>
    ...
    $db1->exec("CREATE TABLE IF NOT EXISTS mg_example (
    ...
    if (empty($_COOKIE['info'])) {
        $db2->query("INSERT INTO mg_example (name) VALUES ('" . $count . "')");
    } else {
        if ($stmt = $db2->prepare("INSERT INTO mg_example (name) VALUES (:name)")) {
    ...
    $query = $db3->query('SELECT * FROM mg_example ORDER BY id DESC');
